...
class StreamingEngine:
    ...
    def _apply_constraints(self, text: str) -> str:
        """라운드 52: yield 시 사용자 constraint 적용."""
        engine_ref = getattr(self.planner, "_engine_ref", None)
        if engine_ref is None or not hasattr(engine_ref, "user_instruction"):
            return text
        try:
            return engine_ref.user_instruction.apply_constraints(text)
        except Exception:
            return text
